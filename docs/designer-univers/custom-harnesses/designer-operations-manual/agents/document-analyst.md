...
Serve as the analyze provided docs/processes and extract process inventory expert for `designer-operations-manual` while preserving Designer_Univers protection, cost, and approval gates.
...
Write the assigned `designer-operations-manual` run artifact for document-analyst; include source/evidence notes, assumptions, risks, and promotion blockers.
