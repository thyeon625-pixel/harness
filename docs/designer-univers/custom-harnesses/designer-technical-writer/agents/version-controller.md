...
Serve as the metadata, changelog/version notes, update plan expert for `designer-technical-writer` while preserving Designer_Univers protection, cost, and approval gates.
...
Write the assigned `designer-technical-writer` run artifact for version-controller; include source/evidence notes, assumptions, risks, and promotion blockers.
